...
def is_light(color) -> bool:
    """Return whether the color is light enough for dark text on top.

    Perceived brightness is computed with the ITU-R BT.601 luma formula:
        Y = 0.299 * R + 0.587 * G + 0.114 * B

    https://stackoverflow.com/a/1855903

    "150" threshold is originally AI-generated, and I couldn't find a good resource about it.
    ...
    * https://alienryderflex.com/hsp.html
    ...
    # TODO:
    #   Can Processing's brightness() be used here?
    #   https://processing.org/reference/brightness_.html

    r, g, b = to_rgb(color.value)
...
# FIXME: This sketch is AI-generated and I didn't check each and every detail of it yet.
class NamedColorPreviews(BaseSketch):
    ...
    def filter_colors(self, all_colors) -> list[Enum]:
    ...
    def draw_search(self) -> None:
        ...
        if self.query:
            ...
            self.text(
                "×",  # noqa: RUF001
                field_x + field_w - self.INFO_PADDING,
                field_y + field_h / 2,
            )
    ...
    def draw_row(self, color: Enum, y: float, height: float) -> None:
        ...
        self.fill(0 if is_light(color) else 255)
        self.text_align(self.LEFT, self.CENTER)
        self.text(
            color.name,
            self.INFO_PADDING,
        ...
        self.text(
            color.value,
            self.width - self.SCROLLBAR_WIDTH - self.INFO_PADDING,
            y + height / 2,
        )
        self.text_align(self.CENTER, self.CENTER)
